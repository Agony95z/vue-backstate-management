...
const applyList = computed(()=> (checksStore.applyList as any).filter((v: any)=> (v.state === approverType.value || defaultType === approverType.value) && (v.note as string).includes(searchWord.value)))
...
const submitForm = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate(async (valid) => {
    if (valid) {
      ...
      const res = await checksStore.postApply(ruleForm)
      if(res.data.errcode === 0){
        const ret = await checksStore.getApply({ applicantid: usersInfos.value._id })
        if (ret.data.errcode === 0) {
          checksStore.updateApplyList(ret.data.rets)
        }
        // todo
        // putRemind
